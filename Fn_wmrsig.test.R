...
wmrsig.twos<-function(indata, s, r, n, n.surrog=100, surr.test=FALSE, surr.type="fft", tsranges=c(0,Inf)){
  ...
  #make coefficients
  X1<-wmodel(trans0f = t(resp),transnf = lapply(covs.list,t),n=n,freqs = 1:nrow(resp)) #calculate coefficients
  coefs<-list()
  for(i in 1:ncol(X1)){
  ...
  if(surr.test){ 
    ...
    for(j in 1:length(surrog)){
      ...
      X1s<-wmodel(trans0f = t(resp),transnf = lapply(covs.list,t),n=n,freqs = 1:nrow(resp)) #make coefficients
      coefs<-list()
      for(i in 1:ncol(X1s)){
